...
class c_lap_beli extends CI_Controller
{
    ...
    {
        $data['tahun'] = $this->m_lap_beli->gettahun();

        $judul = [
            'title' => 'Omara Clothing Store',
            'head' => 'Laporan Pembelian Produk',
            'username' => $this->session->userdata('username')
        ];

        $this->load->view('manager_header', array_merge($data, $judul));
        $this->load->view('laporanPembelian/laporan_Pembelian', $data);
    ...
    public function filter()
    {
        ...
        if ($nilaifilter == 1) {
            $data['title'] = "Laporan Pembelian Produk";
            $data['subtitle'] = "Periode " . date_format($dateawal, "d/m/Y") . " - " . date_format($dateakhir, "d/m/Y");
            $data['filter'] = $this->m_lap_beli->filterbytanggal($tanggalawal, $tanggalakhir);

            $this->load->view('laporanPembelian/print_laporanPembelian.php', $data);
            ...
            $data['filter'] = $this->m_lap_beli->filterbybulan($tahun1, $bulanawal, $bulanakhir);

            $this->load->view('laporanPembelian/print_laporanPembelian.php', $data);
        } else if ($nilaifilter == 3) {
            $data['title'] = "Laporan Pembelian Produk Berdasarkan Tahun";
            $data['subtitle'] = " Tahun : " . $tahun2;
            $data['filter'] = $this->m_lap_beli->filterbytahun($tahun2);
            $this->load->view('laporanPembelian/print_laporanPembelian.php', $data);
        }
    }
}
